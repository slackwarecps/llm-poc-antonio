...
import chatgpt
from chatgpt import func_gpt_submit_tool, func_gpt_busca_runs_ativas
from chatgpt import func_gpt_busca_mensagens, func_gpt_rodar_assistente
...
def teste():
    ...
    dynamo_mensagem_salvar(
        "5519981857976", "thread_h0tWPIgEucMek0aNRTbrdGVq", "alguma coisa")
...
def teste_slot1():
    ...
    thread_id = 'thread_JOggwKFkf3I1Dv9lO9SOmZfd'
    retorno = func_gpt_busca_runs_ativas(thread_id)
...
def teste_slot7():
    ...
    thread_id = data_json['dados']['thread_id']
    telefone = data_json['dados']['telefone']
    assistant_id = data_json['dados']['assistant_id']
    retorno = func_gpt_rodar_assistente(thread_id, telefone, assistant_id)

    logging.info('=============================')
    return (retorno, 201)


@app.route(api+'teste/slot8', methods=['POST'])
def teste_slot8():

    data_json = request.get_json()
    logging.info('POST >> teste/slot8')

    logging.info(data_json)
    objeto = {
        'dados': data_json['dados']
    }
    remetente = data_json['dados']['remetente']
    mensagem = data_json['dados']['mensagem']
    destino = data_json['dados']['destino']
    retorno = func_responde_ao_cliente_pelo_whatsapp(
...
@app.route(api+'teste/slot9', methods=['POST'])
def teste_slot9():

    data_json = request.get_json()
    logging.info('POST >> teste/slot9')

    thread_id = data_json['dados']['thread_id']
    last_created = data_json['dados']['last_created']

    lista_de_mensagens_full = func_gpt_busca_mensagens(thread_id)
    ...
    if 'data' in payload_json:
        ...
        for item in data2:
            if item['role'] == 'assistant':
                # logging.info('>>> Agora epoch: '+str(agora_epoch))
                logging.info('>>> last_created epoch: '+str(last_created) +
                             ' /  ' + func_converteDataPochToBR(last_created))

                logging.info(str(item['created_at']) + ' => ' +
                             func_converteDataPochToBR(int(item['created_at'])))
                if int(item['created_at']) >= last_created:
                    logging.info(item)
                    mensagem_lista.append(item)
                else:
                    logging.info('mais antigo.. nao vou mostrar')

    retorno = jsonify(mensagem_lista)

    logging.info('=======xxxxxxxxxxxxxxxxxx ======================')
    return (retorno, 201)


@app.route(api+'teste/slot10', methods=['POST'])
def teste_slot10():
    dados = [
        {'nome': 'Item 1', 'categoria': 'categoria A'},
        {'nome': 'Item 2', 'categoria': 'categoria B'},
        {'nome': 'Item 3', 'categoria': 'categoria C'}
    ]

    # Aqui vem a piada de programador: sempre que você encontrar um bug, lembre-se...
    dados.append({'piada': 'Eu nunca conto bugs, eu apenas os deixo sair.'})
